...
const map = new _modules_GoogleMap__WEBPACK_IMPORTED_MODULE_3__["default"]();
...
const searchResults = async searchValue => {
  try {
    ...
    resultsContainer.innerHTML = `
		...
		${newData.generalInfo.length ? `<ul class="link-list min-list">
			${newData.generalInfo.map(result => `<li><a href="${result.permalink}">${result.title}</a> ${result.author ? `By: ${result.author}` : ""}</li>`).join("")} </ul>` : "<p>No results to show</p>"}
</div>

		<div class="one-third">
		<h2 class="search-overlay__section-title">Programs</h2>
		${newData.programs.length ? `<ul class="link-list min-list">
			${newData.programs.map(result => `<li><a href="${result.permalink}">${result.title}</a></li>`).join("")} </ul>` : `<p>No programs match that search <a href="${university.root}/programs">View All Programs</a> </p>`}
		<h2 class="search-overlay__section-title">Professors</h2>
		${newData.professors.length ? `<ul class="professor-cards">
			${newData.professors.map(result => `<li class="professor-card__list-item"><a class="professor-card" href="${result.permalink}">
						<img class="professor-card__image" src="${result.image}" alt="">
						<span class="professor-card__name">${result.title}</span>
					</a></li>`).join("")} </ul>` : "<p>No results to show</p>"}
		</div>
		<div class="one-third">
		<h2 class="search-overlay__section-title">Campuses</h2>
		${newData.campuses.length ? `<ul class="link-list min-list">
			${newData.campuses.map(result => `<li><a href="${result.permalink}">${result.title}</a></li>`).join("")} </ul>` : `<p>No campuses match that search <a href="${university.root}/campuses">View All Campuses</a> </p>`}
		<h2 class="search-overlay__section-title">Events</h2>
		${newData.events.length ? `
			${newData.events.map(result => `<div class="event-summary">
...
</div>`).join("")} </ul>` : `<p>No events match that search <a href="${university.root}/events">View All Events</a> </p>`}
		</div>
		</div>
</div>`;
  } catch (error) {
...
searchInput.addEventListener("input", e => {
  ...
  if (searchInput.value !== "" && newSearchValue.length >= 3 && newSearchValue !== storedSearchValue) {
    if (!spinnerVisible) {
...
document.addEventListener("keydown", e => {
  if ((e.key == "s" || e.key == "S") && isOpen == false && !inputTags.includes(document.activeElement.tagName)) {
    isOpen = true;
    openSearch();
    return;
  }
